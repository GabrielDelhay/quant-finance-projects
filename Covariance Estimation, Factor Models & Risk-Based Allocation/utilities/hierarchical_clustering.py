...
def hierarchical_clustering(
    ...
    # `data` is a square pairwise-distance matrix, not a set of raw observations:
    # it must be condensed before scipy.linkage, otherwise scipy silently
    # reinterprets each row as an observation vector and clusters on the Euclidean
    # distance between rows -- a different, second-order quantity.
    condensed = squareform(data, checks=False)
    linkage_matrix = sch.linkage(condensed, method=linkage_method)
    if plot_dendrogram:
        plt.figure()
        sch.dendrogram(
            linkage_matrix,
